...
from utils import recall_at_k, ndcg_k, get_metric, get_user_seqs, nCr, GRL
...
class Trainer:
    def __init__(self, model, train_dataloader,
        ...
        self.optim = Adam([{'params': self.model.parameters()}, {'params': self.stability_discriminator.parameters()},
                           {'params': self.augmentation_discriminator.parameters()}], lr=self.args.lr, betas=betas, 
...
class CoSeRecTrainer(Trainer):
    ...
    def augmentation_discriminator_loss(self, inputs, lambda_1=0.5):
        """
        计算增强判别器的损失
        ...
        # 计算增强判别器的对比损失
        similarity_score = torch.cosine_similarity(cl_output_slice[0], cl_output_slice[1], dim=-1)
        log_prob = torch.log(similarity_score + 1e-8)  # 避免数值问题
        loss_similarity = -torch.mean(log_prob)

        # 计算时间间隔分布的 KL 散度
        kl_div = F.kl_div(cl_output_slice[0].log(), cl_output_slice[1], reduction='batchmean')

        # 总损失
        loss = loss_similarity + lambda_1 * kl_div
        return loss

    def stability_discriminator_loss(self, inputs, lambda_2=0.5):
        """
        计算稳定性判别器的损失
        ...
        # 计算时间间隔方差之差
        var_orig = torch.var(cl_output_slice[1], unbiased=False)
        var_aug = torch.var(cl_output_slice[0], unbiased=False)
        var_loss = torch.abs(var_orig - var_aug)

        # 总损失
        loss = loss_similarity + lambda_2 * var_loss
        return loss
        ...
        if mode == 'train':
            ...
            for i, (rec_batch, cl_batches) in rec_cf_data_iter:
                ...
                for cl_batch in cl_batches:
                    ...
                    +self.augmentation_discriminator_loss(cl_batch)
                    +self.stability_discriminator_loss(cl_batch)
